...
- `sql-injection-body` rule: high-confidence body-only SQLi (adjacent UNION SELECT, tautologies, timing functions, `xp_cmdshell`, `load_file`, `INTO OUTFILE`), so the main `sql-injection` rule can allow prose in bodies without losing detection.
- `nosql-operator-injection` rule (`rules.json`/`rules-browser-friendly.json`): matches a quoted `"$op":` key or `[$op]` parameter form instead of the bare words (`count`, `find`, `db`) the removed `nosql-injection-attacks` matched in any body.
- `auth-jwt-alg-none` rule: matches the base64url encodings of an `alg:none` JWT header as they actually appear on the wire (replacing a rule that looked for plaintext `"alg":"none"`, which never occurs in an encoded token).
