...
### Fixed
- **Stopped two rules from blocking ordinary traffic** ([#185](https://github.com/fabriziosalmi/caddy-waf/pull/186)). In the shipped `rules.json`/`rules-browser-friendly.json`:
...
- Made the CI benchmark regression gate robust to shared-runner noise (warm-up before measuring, 2× threshold), so it no longer false-fails perf-neutral PRs ([#185](https://github.com/fabriziosalmi/caddy-waf/pull/186)).
