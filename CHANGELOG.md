...
- `sql-injection-body` rule: high-confidence body-only SQLi (adjacent UNION SELECT, tautologies, timing functions, `xp_cmdshell`, `load_file`, `INTO OUTFILE`), split out of `sql-injection` so the main rule can allow prose in bodies without losing detection.

### Changed
- **Rewrote and renamed rules that were dead or too broad, under new IDs** (these are rewrites of deleted rules, not new capabilities):
  - `log4j-14`/`log4j-15`/`log4j-16` → `log4shell-jndi`: the old rules had a mid-pattern `$` anchor and never fired; the replacement catches `${…jndi…}`, `jndi:` schemes, and the nested `${${lower:j}ndi…}` obfuscation without matching CI syntax like `${{ inputs.x }}`.
  - `nosql-injection-attacks` → `nosql-operator-injection`: matches a quoted `"$op":` key or `[$op]` parameter form instead of the bare words (`count`, `find`, `db`) the old rule matched in any body.
  - `auth-jwt-algorithm-none` → `auth-jwt-alg-none`: matches the base64url encodings of an `alg:none` JWT header as they appear on the wire (the plaintext `"alg":"none"` the old rule looked for never occurs in an encoded token).
  - The 13 `ssti-*` rules → 5: the byte-identical `{{…}}`, `{%…%}`, `${…}`, `<%…%>` duplicates are consolidated (a single `${…}` used to stack 21–28 points and block CI YAML), scored below the threshold.
  - `rce-java-exec`: rewritten to catch `Runtime.getRuntime().exec(`, `Runtime.exec(`, and `ProcessBuilder(` — the last previously uncovered. The old pattern's trailing `\s*\(` on every branch required a double paren and never matched. OS-reconnaissance coverage (`uname`/`whoami`/`hostname`/`systeminfo`/`ifconfig`/`ipconfig`/`netstat`), previously in the FP-prone `rce-os-info-commands`, is recovered as separator-anchored entries in `rce-command-separators` and `rce-commands-expanded`. (Remote-URL RFI was deliberately not re-added: blocking any external `http(s)://` in a parameter breaks OAuth redirects and webhooks.)
- **Rules audit, part 2 of 4: rewrote the surviving `rules.json` / `rules-browser-friendly.json` rules.**
  - `sql-injection` now allows content between keyword pairs but not a `&` (real `SELECT col1, col2 FROM t` is caught; `?select=name&from=2024` is not), adds string tautologies (`' OR 'a'='b`), `OR true/false`, `waitfor delay` (the old pattern demanded a paren T-SQL never uses), `pg_sleep`, `load_file`, `INTO OUTFILE`. BODY moved to the new stricter `sql-injection-body` rule, so prose like "select an item from the list" no longer blocks.
...
### Fixed
- **The rule `action` field was never read.** `Rule.Action` was tagged `json:"mode"`, but every shipped rule file uses the key `"action"`, so `Action` unmarshalled to the empty string for every rule. Consequences: `action: "block"` never triggered an explicit block (all blocking happened only by anomaly-score accumulation crossing `anomaly_threshold`), and any code branching on the action (including the log-mode change above) was dead. The tag is now `json:"action"`. With it wired up, `block` rules block on match as intended, and — because no shipped `block` rule is scored below the threshold — this does not newly block any request that was not already blocking via accumulation. Found by a live end-to-end run against a built Caddy binary; the Go tests missed it because several set `Action` directly on the struct, bypassing JSON.
...
- Removed `rules/data-validation.json` (both surviving rules — a 15+ digit number and a >5000-char string — logged ordinary Snowflake IDs and minified JSON) and `rules/authentication.json`'s response-text rule.
