...
### Security
- **Log redaction of inspected traffic is value-aware and on by default.** Redaction previously keyed only on the rule *target name* (`password`/`token`/…), so a secret carried in a generic target (`ARGS`, `BODY`, the full `HEADERS` blob) was written verbatim even with `redact_sensitive_data` enabled. It now also redacts when the *value* carries a sensitive marker, and `redact_sensitive_data` is a nullable bool that defaults **on** for both Caddyfile and JSON (disable with `redact_sensitive_data off` / JSON `false`).
- **Rule load fails closed on a partial *initial* load.** If a configured rule file cannot be loaded at startup, the WAF now refuses to start instead of coming up with a partial rule set (the reload fail-safe that keeps the previous set is unchanged).
- **`block_oversize_request_body`** (off by default): a request body larger than `max_request_body_size` was forwarded to the upstream with only its prefix inspected, so a payload placed past the window evaded body rules. The oversize case is now always logged, and this directive makes it fail closed.
- **Phase 3/4 (response) blocks now set the response status.** The recorder holds the status in inspect mode until response-phase evaluation completes, so a response-phase block delivers its configured status instead of the upstream's (typically `200`).
- **`metrics_allow_from`**: optional IP/CIDR allow-list restricting the dashboard/metrics/prometheus endpoints (default: no built-in restriction, as before).
- **`anomaly_threshold` has a single default (`5`)** shared by the Caddyfile and JSON sources (JSON previously defaulted to `20`, silently four times more permissive).
- `docker-compose.yml` no longer binds Caddy's unauthenticated admin API to `0.0.0.0:2019` (back to the loopback default).

### Added
- **`caddywaf_rules_loaded`** Prometheus gauge and `rules_loaded` JSON metric, so an inert WAF (zero rules) is distinguishable from clean traffic — alert on `== 0`.
- CI runs the test suite under the **race detector** (`go test -race`), and a **govulncheck** workflow surfaces advisories on push/PR/weekly.
- `rate_limit { max_entries }` (default 1,000,000) bounds the rate-limiter key table so a flood of distinct source IPs cannot grow it without limit.
...
### Changed
- **File-watcher goroutines are torn down on `Shutdown`.** They previously had no cancellation, so every Caddy reload leaked a watcher goroutine and its inotify handle bound to the defunct module; they are now bound to the module via a stop channel and waited on at shutdown.
- **The Tor exit-node blacklist is written atomically** (temp file → fsync → rename) instead of a truncate-in-place write, so a crash mid-write can no longer leave a partial (silently shorter) blacklist.
- **Hot-path Debug logging no longer allocates when disabled** (`logger.Check`/`ce.Write`), removing a per-rule/per-target field-slice allocation at the default log level.
- Build/packaging: the Dockerfile pins `xcaddy` (`v0.4.7`) and the runtime base (`alpine:3.21`) and no longer bakes a GeoLite2 database from a community mirror; the release build's `-ldflags -X` now targets a real `ModuleVersion` symbol so the git tag is embedded in release binaries.
- `Rule.Action` is a named type (`RuleAction`) with `block`/`log` constants, and `handlePhase` was simplified by extracting the match dispatch.

### Fixed
- Bumped the docs dev-toolchain `nanoid` past its advisory (`npm audit fix`).
- Corrected a stale `.fossa.yml` dependency inventory and the `docs/introduction.md` claim that the WAF has no native Prometheus endpoint (it does).

### Changed (housekeeping)
- Bumped version constant `wafVersionDefault` to `v0.4.15`.
