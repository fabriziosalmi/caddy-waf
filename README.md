...
- **Current version**: `v0.4.3` (see [`caddywaf.go`](caddywaf.go) — `const wafVersion`)
...
```bash
docker pull ghcr.io/fabriziosalmi/caddy-waf:0.4.3
docker run --rm -p 8080:8080 ghcr.io/fabriziosalmi/caddy-waf:0.4.3
```

Tags are `0.4.3`, `0.4` and `latest` — note there is **no `v` prefix**, unlike the Go module version. Pin an exact version in anything you deploy. See [`docs/docker.md`](docs/docker.md) for volumes, Compose, and hot reload.
