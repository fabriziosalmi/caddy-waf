...
3. On match:
   ...
   - The rule's `score` is added to the blocking score `state.TotalScore` **unless** the rule's `mode` is `"log"`, in which case it is added to `state.AdvisoryScore` instead and does not affect the blocking decision (see `log_scores_block` to change this).
